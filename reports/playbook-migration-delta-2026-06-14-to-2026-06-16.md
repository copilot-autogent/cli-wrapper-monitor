...
|--------|--------------|----------------|--------|--|
| systemPromptChars | 226,953 chars | 156,244 chars | −30.1% | ✅ |
...
| **Total** | **226,932 chars** | **156,527 chars** | **−70,405 (−31.0%)** |

> Note: CONTEXT.md grew +14,183 chars between June 14 and June 16 as new gotchas were
> added (empirical-first, self-skip escalation, security checklist, etc.). This partially
> offsets the PLAYBOOK reduction. Without that growth the net reduction would have been
> ~84k chars (−37%).
...
All archived sections are reachable via `recall_memory("topic-name")`. No operational
rules were removed — only narrative context and postmortem details were moved to memory.
...
| No operational rules removed | ✅ verified — only narrative/postmortem sections archived |
...
3. **Post-migration includes post-June-14 growth** — CONTEXT.md and USER.md received updates
   ...
   state as of 2026-06-16, not immediately after the migration. Net effect: actual token savings
   at migration time were ~17,677; subsequent additions partially offset future sessions.
