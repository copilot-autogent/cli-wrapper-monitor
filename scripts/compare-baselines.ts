...
function numRow(label: string, valA: number | undefined, valB: number | undefined, unit = ""): string {
  ...
  let delta: string;
  if (valA !== undefined && valB !== undefined) delta = deltaStr(valA, valB) + severityIcon(valA, valB);
...
function generateMarkdownReport(snapA: MetricSnapshot, snapB: MetricSnapshot): string {
  ...
  for (const expName of experimentNames) {
    ...
    for (const key of metricKeys) {
      ...
      else if (mA && mB) lines.push(`| ${key} | ${mA.value.toLocaleString()} ${mA.unit} | ${mB.value.toLocaleString()} ${mB.unit} | ${deltaStr(mA.value, mB.value) + severityIcon(mA.value, mB.value)} |`);
    }
    lines.push("");
  }
  ...
  lines.push("---", "");
  if (report.hasRegressions) lines.push("🔴 **Regression detected** — one or more metrics changed by >10%.");
  else if (report.changes.some((c) => c.severity === "warning")) lines.push("🟡 **Warning** — one or more metrics changed by 5–10%.");
  else lines.push("✅ **No regression** — all metric changes within the 5% info threshold.");
  lines.push("",
    "| Severity | Threshold |",
    "|----------|-----------|" ,
    "| ⚪ Info | < 5% change |",
...
function main(): void {
  ...
  let content: string;
  if (jsonMode) { content = JSON.stringify(diffSnapshots(snapA, snapB), null, 2); }
