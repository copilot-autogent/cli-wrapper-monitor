...
function main(): void {
  ...
  const annotations = loadAnnotations("notes");
  ...
  if (matrixMode) {
    ...
    const lines = [
      ...
      "",
      `_Deltas relative to current snapshot (${currentDate}). Reference dates per window:_`,
      "",
    ];
    for (let i = 0; i < matrix.windows.length; i++) {
      const w = matrix.windows[i];
      const refs = matrix.rows[0]?.cells[i]?.referenceDate;
      lines.push(`- **${w.label}** (${w.days}d): ${refs ?? "no data"}`);
    }
    lines.push("");
    lines.push(buildTrendMatrixMarkdown(matrix));
    lines.push("");
    lines.push("---");
    lines.push("");
    ...
    report = generateTrendReport(snapshots, annotations, verbose);
  }
